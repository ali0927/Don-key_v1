...
class AuctionContract {
  ...
  async anounceWinners(winners: string[], amounts: string[] ){
      return await this.contract.methods.announceWinnerList(winners, amounts);
  }
  ...
  async initialize() {
    const contractAbi = await import("../Abis/AuctionContract.json");
