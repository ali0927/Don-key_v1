...
import { generateGradientImage, uuidv4 } from "don-utils";


const saveStrategy = async (strategy: IStrategy, onSave?: () => Promise<void>) => {
...
export const useGraphState = ({ strategy, getGraph, getProtocol }: {
  ...
  const insertAction = async (prevCellId: string, nextCellId: string, actionName: string) => {
    ...
    const image = generateGradientImage(protocol.edgeColor, nextProcotol.edgeColor)
