...
import { useSetRef, useToggle } from "don-hooks";
...
export const GraphProvider = ({ children, openPanel, protocols, strategy }: any) => {
  const [getSelectedCell, setSelectedCell] = useSetRef({});

  const getProtocol = (name: string) => {
    const index = findIndex(protocols, (item: any) => item.name === name);
    if (index === -1) {
      return null;
    }
  ...
  const getGraph = useCallback(() => {
    return graphRef.current as any;
  }, []);
  const {
    getActionCells,
    getProtocolCells,
  ...
  const getPrevProtocol = () => {
    return getProtocolCells()[
      getProtocolCells().findIndex(
        (item: any) => item.protocolId === getSelectedProtocol().protocolId
      ) - 1
    ];
  }
  ...
  useEffect(() => {
    ...
    graph.addMouseListener({
      mouseDown: function (sender: any, evt: any) {
      ...
      mouseUp: function (sender: any, evt: any) {
    ...
    graph.addListener("size", function () {
      ...
      graph.view.states.visit(function (key: any, state: any) {
    ...
    mx.mxEvent.addListener(container, "drop", function (evt: any) {
      if (graph.isEnabled()) {
        evt.stopPropagation();
        evt.preventDefault();
    ...
    mx.mxEvent.addListener(container, "dragover", function (evt: any) {
      if (graph.isEnabled()) {
        evt.stopPropagation();
        evt.preventDefault();
      }
