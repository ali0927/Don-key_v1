...
export const UniswapSwapToken = ({}: {
  ...
  const [
    inputCurrency,
    setInputCurrency,
  ] = useState<ICurrencyWithAddress | null>(null);
  const [
    outputCurrency,
    setOutputCurrency,
  ] = useState<ICurrencyWithAddress | null>(null);
  const [{ midPrice, invertedMidPrice }, setPriceRatio] = useState({
    midPrice: "0",
    invertedMidPrice: "0",
  });
  ...
  useEffect(() => {
    (async () => {
      if (inputCurrency && outputCurrency) {
        const resp = await api.post("/api/v1/protocols/uni", {
          address1: inputCurrency.address,
          address2: outputCurrency.address,
        });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value === "") {
      ...
      setOutPutAmount(
        parseFloat(e.target.value) * parseFloat(invertedMidPrice)
      );
    }
  };
