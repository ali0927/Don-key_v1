import { Pagination, Row, Container, Col, Button, FormGroup, Spinner, } from "react-bootstrap";
import { VoteIcon } from "icons/VoteIcon";
import * as React from "react";
import { Form, Table } from "react-bootstrap";
import "../DevelopersPage.scss";
import { useAxios } from "hooks/useAxios";
import _, { forEach, forOwnRight } from "lodash";
    ...
    return (
        <>
                ...
                <Table responsive>
                    ...
                    <tbody>
                        ...
                        {([...cubes].slice(startCube, endCube)).map((cube, index) => {
                            const isVotted = uservottedcubes.findIndex(x => x.devcubeid === cube.id) > -1;
                            return (
                                <>
                                    <tr key={index}>
                                        <td><img src={cube.iconname} alt="cube icon" /></td>
                                        <td>{cube.protocol}</td>
                                        <td><a href={cube.githublink} className="toBodyAnchor" target="_blank">{cube.githublink}</a></td>
                                        ...
                                        <td>
                                            ...
                                            {!isVotted &&
                                                <LikeIcon className="voteIcon" onClick={handleVotes(cube.id)} />
            ...
            <div className="mt-5 mb-5 pagePosition d-flex justify-content-between">
                <p className="pageTable">{`Showing ${startCube + 1}-${endCube > rowsCount ? rowsCount : endCube} of ${rowsCount}`}</p>

                <div className="paginationTable">
                    <Pagination>
                        <Pagination.Prev className="paginationfocus" onClick={handleNextPrev("prev")} />
                        ...
                        <Pagination.Next className="paginationfocus" onClick={handleNextPrev("next")} />
                    </Pagination>
                </div>
                ...
                <div className="dropTable">
                    ...
                    <Form.Group>
                        <Form.Control as="select" value={selectedRow} onChange={handleRowChange}>
                            {rows.map((row, index) => {
