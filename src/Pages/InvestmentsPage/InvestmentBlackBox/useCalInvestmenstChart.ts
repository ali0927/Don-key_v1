import { calculateInitialInvestment, calculateWithdrawAmount, getROI, getProfitLoss } from "helpers";
import { useEffect, useState } from "react";
import { IMyInvestments } from "../interfaces";


export const useCalInvestmentsChart = (web3: any,myInvestments: IMyInvestments[]) => {
    ...
    useEffect(() => {
        async function apiCall() {  
          ...
          for(let investment of myInvestments){
            ...
            const initialInvestment = await calculateInitialInvestment(web3, investment.poolAddress);
            // const roivalue= await getROI(web3,investment.poolAddress);
            const profitlossvalue = await getProfitLoss(web3,investment.poolAddress);
            
            //initial invest total
            initialInvestmentsTotal = initialInvestmentsTotal+parseFloat(initialInvestment)
